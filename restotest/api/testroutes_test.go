...
func testGetVersion(url, version string, client *http.Client, t *testing.T) (*http.Response, error) {
	...
	req.Header.Add("Accept", fmt.Sprintf("application/vnd.resto.v%s+json", version))
...
func TestVersionedRoutes(t *testing.T) {
	...
	if rsp.Header.Get("Resto-API-Version") != "v1" {
		t.Errorf("expected API Version header to be v1, got: %s", rsp.Header.Get("Resto-API-Version"))
		t.Fail()
	}
	...
	if rsp.Header.Get("Resto-API-Version") != "v2" {
		t.Errorf("expected API Version header to be v2, got: %s", rsp.Header.Get("Resto-API-Version"))
		t.Fail()
	}
