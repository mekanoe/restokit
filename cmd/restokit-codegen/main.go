...
var packageRe = regexp.MustCompile(`package ([a-zA-Z0-9_]+)`)
...
func parsePackage(path string) string {
	b, err := ioutil.ReadFile(path)
	if err != nil {
		log.Fatalln(err)
	}

	name := packageRe.FindSubmatch(b)[1]

	// fmt.Println("=>", path, "got", string(name))
...
func renderGoFile(tmplPath, cwd string) {
	...
	err = t.ExecuteTemplate(f, "main", generator{
		Routes:      routes,
	...
	err = os.Rename(cwd+"/api.generated.go~", cwd+"/api.generated.go")
	if err != nil {
		log.Fatal(err)
	}
